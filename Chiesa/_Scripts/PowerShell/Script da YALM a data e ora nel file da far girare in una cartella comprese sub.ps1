...
[ITA] 6. AMBITO DI ESECUZIONE: analizza i file .md nella cartella corrente e in tutte le sottocartelle.
...
[ENG] 6. EXECUTION SCOPE: processes .md files in the current folder and in all subfolders.
[ENG] 7. EXECUTION NOTE: when concatenated with other scripts, remove 'Pause' to avoid a second confirmation.
#>

# [ITA] Recupera tutti i file Markdown della cartella corrente e delle sottocartelle.
# [ENG] Retrieves all Markdown files from the current folder and its subfolders.
$files = Get-ChildItem -LiteralPath . -File -Filter "*.md" -Recurse
	...
	foreach ($line in $content) {
		...
		if ($line -match "^(creato|modificato):\s*(\d{2,4}[/-]\d{2}[/-]\d{2,4}\s\d{2}:\d{2}:\d{2})") {
			...
			if ([DateTime]::TryParseExact($dateStr, [string[]]$formats, $culture, $styles, [ref]$parsedDate)) {
				# [ITA] Se il campo e 'creato', conserva la data estratta.
				# [ENG] If the field is 'creato', stores the extracted date.
				if ($tipo -eq "creato") {
				...
				# [ITA] Se il campo e 'modificato', conserva la data estratta.
				# [ENG] If the field is 'modificato', stores the extracted date.
